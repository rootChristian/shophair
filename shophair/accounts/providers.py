...
def registerSocialUser(provider, email, first_name, last_name):
    old_user=User.objects.filter(email=email)
    if old_user.exists():
        if provider == old_user[0].auth_provider:
            register_user=authenticate(email=email, password=base.SOCIAL_AUTH_PASSWORD)
        ...
        user.save()
        login_user=authenticate(email=email, password=base.SOCIAL_AUTH_PASSWORD)
